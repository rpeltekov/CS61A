...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        new_procedure = scheme_eval(first, env)
        check_procedure(new_procedure)

        if type(new_procedure) is MacroProcedure:
            return scheme_eval(new_procedure.apply_macro(rest, env), env, True)
        return scheme_apply(new_procedure, 
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('val number mismatch to number of formals')
...
def do_or_form(expressions, env):
    ...
    while expressions is not nil:
        if expressions.rest is nil:
            truthfulness = scheme_eval(expressions.first, env, True)
        else:    
            truthfulness = scheme_eval(expressions.first, env)
        if not scheme_truep(truthfulness):
...
def make_let_frame(bindings, env):
    ...
    def populate_a_pair(pair_list, val):
        if pair_list.rest is nil:
            pair_list.rest = Pair(val, nil)
        else:
            populate_a_pair(pair_list.rest, val)

    #initialize the formals and values Pairs
    formals = Pair(nil, nil)
    values = Pair(nil, nil)
    
    #do the first formals and values Pairs
    if bindings is not nil:
        check_form(bindings.first, 2, 2)
        formals.first = bindings.first.first
        values.first = scheme_eval(bindings.first.rest.first, env)
        bindings = bindings.rest

        #do the rest of the values and pairs
        while bindings is not nil:
            check_form(bindings.first, 2, 2)
            populate_a_pair(formals, bindings.first.first)
            populate_a_pair(values, scheme_eval(bindings.first.rest.first, env))
            bindings = bindings.rest
        check_formals(formals)
