...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        if not self.direction_exit and not self.has_been_scared:
    ...
    def reduce_armor(self, amount):
        ...
        for current_bee in list(current_place.bees):
            for actual_bee in current_place.bees:
                if actual_bee == current_bee:
                    actual_bee.reduce_armor(attack_damage)
        # END Problem 5
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        ...
        for current_bee in list(current_place.bees):
            for actual_bee in current_place.bees:
                if actual_bee == current_bee:
                    actual_bee.reduce_armor(self.damage)        # END Problem 7
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        ...
        for current_bee in list(current_place.bees):
            for actual_bee in current_place.bees:
                if actual_bee == current_bee:
                    actual_bee.reduce_armor(self.damage)
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        if not QueenAnt.there_is_instance:
            print("DEBUG:", "true queen made")
            self.is_true_queen = True
            QueenAnt.there_is_instance = True
        else:
            print("DEBUG:", "imposter queen made")
        ...
        if self.is_true_queen:
            ...
            while current_place.exit:
                if current_place.ant:
                    if not current_place.ant.is_damage_buffed:
                        current_place.ant.damage *= 2
                        current_place.ant.is_damage_buffed = True
                    if current_place.ant.is_container:
                        if current_place.ant.contained_ant is not None:
                            if not current_place.ant.contained_ant.is_damage_buffed:
                                current_place.ant.contained_ant.damage *= 2
                                current_place.ant.contained_ant.is_damage_buffed = True
...
def make_slow(action, bee):
    ...
    def slow_action(colony):
        print("DEBUG:", "COLONY TIME:", colony.time)
...
def make_scare(action, bee):
    ...
    """
    # BEGIN Problem EC
    def scary_action(colony):
        bee.direction_exit = False
        action(colony)
    return scary_action
    # END Problem EC
...
def apply_effect(effect, bee, duration):
    ...
    new_action = effect(old_bee_action, bee)

    def effect_applier(colony):
        nonlocal duration
        ...
        if duration > 0:
            ...
            print("DEBUG:", old_bee_action)
            old_bee_action(colony)

    bee.action = effect_applier
    # END Problem EC
