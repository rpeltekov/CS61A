...
def make_counter():
    ...
    set_stored_things = []
    def helper(thing):
        set_stored_things.append(thing)
        return set_stored_things.count(thing) 
    return helper
...
class VendingMachine:
    ...
    def vend(self):
        if self.stock < 1:
            ...
            remaining = self.balance - self.cost
            self.stock -= 1
            self.balance = 0
            return 'Here is your {0} and ${1} change.'.format(self.name, remaining)
...
def generate_paths(t, x):
    ...
    if t.label == x:
        yield [t.label]
    for b in t.branches:
        for a_path in generate_paths(b, x):
            yield [t.label] + a_path
