...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, pass_try):
        nonlocal balance, password, incorrect_passwords, locked

        if pass_try != password and not locked:
            incorrect_passwords += [pass_try]
        ...
        elif pass_try == password and not locked:
            if amount > balance:
...
def make_joint(withdraw, old_password, new_password):
    ...
    "Your account is locked. Attempts: ['my', 'secret', 'password']"
    """
    tester = withdraw(0, old_password)
    if type(tester) == str:
        return tester

    def joint_withdraw(amount, pass_try):
        if pass_try == old_password or pass_try == new_password:
